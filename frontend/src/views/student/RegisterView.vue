<template>
  <section>
    ...
    <div class="panel form-narrow agri-card" v-loading="loading">
      <el-alert
        v-if="activity && !canRegister(activity)"
        class="qr-tip"
        :title="registerDisabledReason(activity)"
        type="warning"
        :closable="false"
        show-icon
      />
      ...
      <el-form :model="form" label-width="96px">
        ...
        <el-form-item>
          <el-button type="primary" :disabled="activity && !canRegister(activity)" :loading="submitting" @click="submit">提交报名</el-button>
...
import { canRegister, registerDisabledReason, statusText, userCampuses } from '../../utils/options'
...
async function submit() {
  if (activity.value && !canRegister(activity.value)) {
    ElMessage.warning(registerDisabledReason(activity.value))
    return
  }
  submitting.value = true
  try {
    await registerActivity(route.params.id, form)
    ElMessage.success('报名成功，记得按时参加活动并完成签到。')
