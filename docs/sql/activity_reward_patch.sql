...
SET @sql = IF(
  ...
  'ALTER TABLE `activity` ADD COLUMN `activity_category` VARCHAR(50) NOT NULL DEFAULT ''其他'' COMMENT ''活动类型：公益活动/实践活动/志愿服务/讲座培训/文体活动/竞赛活动/社团活动/学院活动/其他'' AFTER `activity_mode`',
  'SELECT 1'
);
...
SET @sql = IF(
  ...
  'ALTER TABLE `activity` ADD COLUMN `reward_enabled` TINYINT NOT NULL DEFAULT 0 COMMENT ''是否设置奖励：1是/0否'' AFTER `allow_cross_campus`',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = IF(
  ...
  'ALTER TABLE `activity` ADD COLUMN `reward_type` VARCHAR(30) NOT NULL DEFAULT ''无'' COMMENT ''奖励类型：课外学时/积分/证书/实物奖励/无'' AFTER `reward_enabled`',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
